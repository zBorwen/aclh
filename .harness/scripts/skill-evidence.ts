...
const POLICY_PATH = process.env.ACLH_SKILL_EVIDENCE_POLICY
  ? path.resolve(ROOT, process.env.ACLH_SKILL_EVIDENCE_POLICY)
  : path.join(ROOT, '.harness/policies/skill-evidence.yaml');
...
try {
  ...
  const statuses = verifyEvidenceGates(loaded.evidence, current, requirements.gates);
  let failed = false;
